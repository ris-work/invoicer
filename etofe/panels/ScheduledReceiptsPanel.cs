...
namespace RV.InvNew.UI
{
    public class ScheduledReceiptPanel : Panel
    {
        ...
        void OnKeyUp(object sender, KeyEventArgs e)
        {
            if (new[] { Keys.F1, Keys.F2, Keys.F3, Keys.F4 }.Contains(e.Key))
            {
                var map = new Dictionary<Control, Action>
                {
                    ...
                    { txtJournalNumber, () => btnJournalNumberSearch.PerformClick() },
                    { txtDebitAccount, () => btnDebitAccountSearch.PerformClick() },
                    { txtDebitAccountType, () => btnDebitAccountTypeSearch.PerformClick() },
                    { txtCreditAccount, () => btnCreditAccountSearch.PerformClick() },
                    { txtCreditAccountType, () => btnCreditAccountTypeSearch.PerformClick() },
                };
        ...
        {
            // Stub for NA fields without a predefined search function
            var items = new List<object> { new { Id = 1L, Name = "Dummy Item 1" }, new { Id = 2L, Name = "Dummy Item 2" } };
            var sel = SearchPanelUtility.GenerateSearchDialog(items, this, order: ["Id"]);
            if (sel?.Length > 0 && long.TryParse(sel[0], out var id))
            {
                tb.Text = id.ToString();
                human.Text = lookup(id);
        ...
        void ClearFields()
        {
            ...
            {
                txtId, txtCompanyId, txtVendorId, txtInvoiceId, txtBatchId, txtPaymentReference, txtDescription,
                txtCurrency, txtAmount, txtExchangeRate, txtBeneficiaryName, txtBeneficiaryBankName,
                txtBeneficiaryBranch, txtBeneficiaryAccountNo, txtBeneficiaryRoutingNo, txtPaymentMethod,
                txtFrequency, txtIntervalValue, txtExternalPaymentId, txtFeeAmount, txtNetAmount, txtReconciliationRef,
                txtCreditAccount, txtCreditAccountType, txtDebitAccount, txtDebitAccountType, txtJournalNumber
            }) tb.Text = "";

            foreach (var lbl in new[]
            {
                 lblIdHuman, lblCompanyHuman, lblVendorHuman, lblInvoiceHuman, lblBatchHuman, lblCreditAccount,
                 lblCreditAccountType, lblDebitAccount, lblDebitAccountType, lblJournalNumber
            }) lbl.Text = "";
