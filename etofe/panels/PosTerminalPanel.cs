...
public class PosPanel : Scrollable
{
    ...
    void InitializeFields()
    {
        ...
        tbCustomer = SearchField<Pii>(
            ListPii,
            out lblCustomerName,
            "Customer PII",
            () => tbSalesPerson
        );
        tbSalesPerson = SearchField<Pii>(
            ListPii,
            out lblSalesPersonName,
            "Sales Person PII",
            () => tbCurrency
        );
        ...
        tbItem = SearchField<Catalogue>(
            () => ListCatalogue(),
            out lblItemName,
            "Item code",
            () => tbInventory
        );
        tbInventory = SearchField<Inventory>(
            () => ListInventory(ParseLong(tbItem.Text)),
            out lblInventoryName,
            "Inventory code",
            () => tbQty
        );
        ...
        rcptGrid.Columns.Add(
            new GridColumn
            {
                HeaderText = "Account",
                DataCell = new TextBoxCell
                {
                    Binding = Binding.Delegate<Receipt, string>(r => r.AccountId.ToString()),
                },
            }
        );
        rcptGrid.Columns.Add(
            new GridColumn
            {
                ...
                DataCell = new TextBoxCell
                {
                    Binding = Binding.Delegate<Receipt, string>(r => r.Amount.ToString("C2")),
                },
            }
        );
        ...
        tbRcptAcct.TextChanged += (_, __) =>
            lblAcctName.Text = LookupHumanFriendlyAccount(tbRcptAcct.Text);
    ...
    void BuildLayout()
    {
        ...
        var paymentPanel = BuildNColumnForm(("Paid:", tbPaid), ("Change:", lblChange));
    ...
    {
        tbCustomer.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter && TryParseLong(tbCustomer.Text, out _))
                tbSalesPerson.Focus();
        };
        tbSalesPerson.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter && TryParseLong(tbSalesPerson.Text, out _))
                tbCurrency.Focus();
        };
        tbCurrency.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter)
                dpInvoiceDate.Focus();
        };
        dpInvoiceDate.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter)
                cbIsPosted.Focus();
        };
        cbIsPosted.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter)
                taRemarks.Focus();
        };
        taRemarks.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter)
                tbItem.Focus();
        };
        ...
        tbItem.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter && TryParseLong(tbItem.Text, out _))
                tbInventory.Focus();
        };
        tbInventory.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter && TryParseLong(tbInventory.Text, out _))
                tbQty.Focus();
        };
        tbQty.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter && TryParseDouble(tbQty.Text, out _))
                tbPrice.Focus();
        };
        tbPrice.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter && TryParseDouble(tbPrice.Text, out _))
                tbDisc.Focus();
        };
        tbDisc.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter && TryParseDouble(tbDisc.Text, out _))
                btnSaveSale.Focus();
        };

        tbRcptAcct.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter && TryParseLong(tbRcptAcct.Text, out _))
                tbRcptAmt.Focus();
        };
        tbRcptAmt.KeyDown += (s, e) =>
        {
            if (e.Key == Keys.Enter && TryParseDouble(tbRcptAmt.Text, out _))
                btnSaveRcpt.Focus();
        };
    }
