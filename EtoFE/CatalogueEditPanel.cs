...
namespace EtoFE
{
    public class CatalogueEditPanel: Panel
    {
        public CatalogueEditPanel()
        {
            ...
            var LowerPanelContent = SimpleJsonToUISerialization.ConvertToUISerialization(JsonSerializer.Serialize(new Catalogue()
            {
                Active = true,
                ActiveWeb = true,
            ...
            var LowerPanel = new GenEtoUI(LowerPanelContent, (a) => { MessageBox.Show(JsonSerializer.Serialize(a), "Serialized", MessageBoxType.Information); return 0; }, (e) => { return 0; }, new Dictionary<string, (CommonUi.ShowAndGetValue, CommonUi.LookupValue)>(), "id", true);
            ...
            SearchBox.OnSelectionMade = () => {
                ...
                var A = SendAuthenticatedRequest<long, Catalogue>.Send(long.Parse(SearchBox.Selected[0]), "CatalogueRead");
                MessageBox.Show(JsonSerializer.Serialize(A.Out), "Response");
                ScrollableLower.Content = new GenEtoUI(SimpleJsonToUISerialization.ConvertToUISerialization(JsonSerializer.Serialize(A.Out)), (a) => { MessageBox.Show(JsonSerializer.Serialize(a), "Serialized", MessageBoxType.Information); return 0; }, (e) => { return 0; }, new Dictionary<string, (CommonUi.ShowAndGetValue, CommonUi.LookupValue)>(), "id", true);
