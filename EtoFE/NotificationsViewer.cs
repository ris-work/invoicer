...
namespace EtoFE
{
    class NotificationsViewer : Eto.Forms.Dialog
    {
        public NotificationsViewer(List<NotificationTransfer> LN)
        {
            ...
            {
                NGI.Add(new GridItem(notification.NotifId, notification.NotifPriority, notification.NotifTarget, notification.TimeTai, notification.NotifContents) { });
            }
            NotificationsGridView.DataStore = NGI;
            
            MessageBox.Show(count.ToString(), "Count", MessageBoxType.Information);
            Title = "Notifications";
            Button MarkAllAsRead = new Button() { BackgroundColor = Eto.Drawing.Colors.DarkRed, TextColor = Eto.Drawing.Colors.White, Text = "Mark as read" };
            Button Exit = new Button() { Text = "Exit" };
            StackLayout SLB = new StackLayout(null, MarkAllAsRead, Exit) { Orientation = Orientation.Horizontal, Spacing = 5, Padding = 5 };
            Content = new StackLayout(NotificationsGridView, SLB) { Orientation = Orientation.Vertical, Padding = 5, HorizontalContentAlignment = HorizontalAlignment.Right };
            Exit.Click += (_, _) => { this.Close(); };
