...
public static class ReceivedInvoiceExtensions
{
    ...
    //-------------------------------------------------------------------------
    public static (bool Valid, string Error) ValidateTotals(
    ...
    )
    {
        ...
        if (!invoice.GrossTotal.AreApproximatelyEqual(computedPurchaseGrossTotal))
        {
            return (
                false,
                $"Rule T1: Invoice GrossTotal ({invoice.GrossTotal}) does not equal sum of purchase GrossTotal ({computedPurchaseGrossTotal})."
            );
        }
    ...
    //-------------------------------------------------------------------------
    public static (bool Valid, string Error) ValidateDiscounts(
        this ReceivedInvoice invoice,
        List<Purchase> purchases
    )
    {
        ...
        {
            errors.Add(
                $"Rule WI1: EffectiveDiscount on invoice ({invoice.EffectiveDiscount}) does not equal sum of purchase discounts ({computedEffectiveDiscount})."
            );
        }
        ...
        {
            errors.Add(
                $"Rule WI2: WholeInvoiceDiscount ({invoice.WholeInvoiceDiscount}) exceeds allowable amount ({totalBeforeDiscount - invoice.EffectiveDiscount})."
            );
        }
        ...
        double expectedDiscountPercentage =
            (totalBeforeDiscount > 1e-6) ? (overallDiscount / totalBeforeDiscount) * 100.0 : 0.0;
        if (!invoice.DiscountPercentage.AreApproximatelyEqual(expectedDiscountPercentage))
        {
            errors.Add(
                $"Rule WI3: DiscountPercentage ({invoice.DiscountPercentage}) does not match expected value ({expectedDiscountPercentage})."
            );
        }
    ...
    /// </summary>
    public static (bool Valid, string Error) Validate(
        this ReceivedInvoice invoice,
        List<Purchase> purchases
    )
    {
