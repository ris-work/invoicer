        ...
        public static void AddCatalogueDefaultImageEndpoints(this WebApplication app)
        {
            app.AddEndpointWithBearerAuth<long>(
                ...
                (AS, P) =>
                {
                    ...
                    {
                        var ImageQ = ctx.InventoryImages.Where(e =>
                            e.Itemcode == ItemCode && e.Imageid == 0
                        );
                        if (ImageQ.Count() > 0)
                        {
                            Image = ImageQ.First();
            ...
            app.AddEndpointWithBearerAuth<InventoryImage>(
                ...
                (AS, P) =>
                {
                    ...
                    using (var ctx = new NewinvContext())
                    {
                        var Image = ctx.InventoryImages.Where(e =>
                            e.Itemcode == InputImage.Itemcode && e.Imageid == 0
                        );
                        if (Image.Count() > 0)
                        {
                        ...
                        {
                            ctx.InventoryImages.Add(
                                new InventoryImage()
                                {
                                    Itemcode = InputImage.Itemcode,
                                    Imageid = 0,
