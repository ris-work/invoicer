...
namespace InvoicerBackend
{
    ...
    {
        public long BucketId { get; set; }    // Maps to LoyaltyPoints.PointsId
        public double Amount { get; set; }    // Points taken from this bucket
    }
            ...
            List<PaymentEntry> payments)
        {
            ...
            foreach (var pay in payments)
            {
                ...
                if (pay.Type == "LP")
                {
                    ...
                    var (success, actualRedeemedAmount, simulationEntries) = VirtualRedeemLp(
                        rawLpBuckets,
                        virtualLpState,
                        pointsToRedeem
                    );
                ...
                else if (pay.Type == "CASH" || pay.Type == "BANK")
                {
                    // VALIDATION
                    if (pay.AccountNo == 0)
                        throw new ArgumentException("Invalid Account ID (0) for Cash/Bank payment.");
