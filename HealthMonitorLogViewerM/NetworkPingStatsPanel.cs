...
namespace HealthMonitor
{
    public class NetworkPingStatsPanel : Panel
    {
        public NetworkPingStatsPanel()
        {
            ...
            var ContainsFilterLabel = new Label()
            {
                Text = "Contains (case-insensitive) (Press [ENTER])",
                TextColor = Eto.Drawing.Colors.White,
            };
            var ContainsFilter = new TextBox()
            {
                TextColor = Eto.Drawing.Colors.White,
                BackgroundColor = Eto.Drawing.Colors.Black,
                ShowBorder = true,
            };
            var ContainsFilterLayout = new StackLayout(ContainsFilterLabel, ContainsFilter)
            {
            ...
            {
                Items = { null, ResetButton, ReloadButton, SaveButton, ContainsFilterLayout, null },
            ...
            foreach (var item in series)
            {
                ...
                {
                    List<PingAverageByDecaminute> pingAveragesByDecaminute =
                        new List<PingAverageByDecaminute>();
                    List<PingSuccessRateByDecaminute> pingSuccessRatesByDecaminute =
                        new List<PingSuccessRateByDecaminute>();
                    using (var logsContext = new LogsContext())
                    {
                        var GroupedByDecaminute = logsContext
                            .Pings.Where((x) => x.Dest == item)
                            .GroupBy((e) => e.TimeNow.Substring(0, 18))
                            .ToList();
                        var GroupedByHour = logsContext
                            .Pings.Where((x) => x.Dest == item)
                            .GroupBy((e) => e.TimeNow.Substring(0, 16))
                            .ToList();
                        Decaminutes = GroupedByDecaminute.Select((e) => e.Key).ToList();
                        pingAveragesByDecaminute = GroupedByDecaminute
                            .Select(e => new PingAverageByDecaminute
                            {
                                Decaminute = e.Key,
                                LatencyAverage = e.Average((x) => x.Latency),
                            })
                            .ToList();
                        pingSuccessRatesByDecaminute = GroupedByDecaminute
                            .Select(e => new PingSuccessRateByDecaminute
                            {
                                Decaminute = e.Key,
            ...
            var RedrawWithFilter = () =>
            {
                ...
                foreach (var item in series)
                {
                    ...
                    if (item.ToLowerInvariant().Contains(AddressContainsFilter.ToLowerInvariant()))
                    {
                        ...
                        using (var logsContext = new LogsContext())
                        {
                            ...
                            pingSuccessRatesByDecaminute = GroupedByDecaminute
                                .Select(e => new PingSuccessRateByDecaminute
                                {
                                    ...
                                    SuccessRate = e.Average(
                                        (x) =>
                                            (x.WasItOkNotCorrupt == 1 || x.DidItSucceed == 1)
                                                ? 1
                                                : 0
                                    ),
                                })
                        ...
                        PlotData.Add(item, pingAveragesByDecaminute);
                        PlotDataSuccessRates.Add(item, pingSuccessRatesByDecaminute);
                        var p = etoPlot.Plot.Add.SignalXY(
                            PlotData[item]
                                .Select(e =>
                                    (DateTime.Parse(e.Decaminute + "0").ToLocalTime().ToOADate())
                                )
                                .ToArray(),
                            PlotData[item].Select(e => e.LatencyAverage ?? 0).ToArray()
                        );
                        var pSuccessRates = etoPlotSuccessRates.Plot.Add.SignalXY(
            ...
            foreach (var item in series)
            {
                var p = etoPlot.Plot.Add.SignalXY(
                    PlotData[item]
                        .Select(e => (DateTime.Parse(e.Decaminute + "0").ToLocalTime().ToOADate()))
                        .ToArray(),
                    PlotData[item].Select(e => e.LatencyAverage ?? 0).ToArray()
                );
                var pSuccessRates = etoPlotSuccessRates.Plot.Add.SignalXY(
            ...
            etoPlot.Plot.FigureBackground = new ScottPlot.BackgroundStyle()
            {
            ...
            etoPlotSuccessRates.Plot.FigureBackground = new ScottPlot.BackgroundStyle()
            {
                Color = ScottPlot.Colors.Black,
            };
